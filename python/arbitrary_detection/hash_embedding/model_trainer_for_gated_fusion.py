...
class TxEncoderGated(nn.Module):
    def __init__(self, num_dim=8, data_vocab_size=60000, logs_vocab_size=80000,
        ...
        # ===== Gated Fusion =====
        # 对每个特征生成一个 gate
        self.gate_num = nn.Linear(emb_dim, emb_dim)
        self.gate_data = nn.Linear(emb_dim, emb_dim)
        self.gate_logs = nn.Linear(emb_dim, emb_dim)
    ...
    def forward(self, num_feat, data_ids, logs_ids):
        ...
        if self.mode == 'attention':
            ...
            data_out, (h_n, _) = self.data_lstm(data_emb)
            logs_out, (h_n_logs, _) = self.logs_lstm(logs_emb)
            data_feat = h_n.squeeze(0)
            logs_feat = h_n_logs.squeeze(0)
            ...
            raise ValueError("mode must be 'attention', 'lstm' or 'cnn'")

        # ==== Gated Fusion ====
        g_num = torch.sigmoid(self.gate_num(num_emb))
        g_data = torch.sigmoid(self.gate_data(data_feat))
        g_logs = torch.sigmoid(self.gate_logs(logs_feat))

        fused = g_num * num_emb + g_data * data_feat + g_logs * logs_feat
        return fused  # (B, D)


# ====== 分类器 ======
class TxClassifier(nn.Module):
    def __init__(self, encoder, emb_dim=128, hidden_dim=64):
        super().__init__()
...
# ====== 训练函数 ======
def train_model(pos_csv, neg_csv, data_vocab_size=60000, logs_vocab_size=80000,
                batch_size=512, epochs=5, lr=1e-3,
                test_ratio=0.2, mode='cnn'):
    ...
    encoder = TxEncoderGated(data_vocab_size=data_vocab_size, logs_vocab_size=logs_vocab_size,
                             num_dim=len(train_set.num_features[0]), mode=mode)
...
if __name__ == "__main__":
    ...
    model = train_model(pos_csv, neg_csv, epochs=30, mode='cnn')
