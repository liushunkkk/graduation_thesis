...
class ChristofHandler:
    """
    处理套利/非套利 CSV 文件交易数据，调用 SwapArbitrageDetector 分析，并计算指标
    """
    ...
    def analyze_files(self, data_csv: str):
        ...
        if os.path.exists("pool_cache.pkl"):
        ...
        for idx, row in df_data.iterrows():
            ...
            if not logs_json or pd.isna(logs_json):
                continue
            try:
                analysis = self.detector.detect_arbitrage(logs_json)
