...
class CharEncoder(nn.Module):
    ...
    def __init__(self, char_emb_dim=16, out_dim=128, mode='cnn'):
        super().__init__()
        self.mode = mode
        self.char_emb = nn.Embedding(16, char_emb_dim)
        print("CharEncoder:", mode)
        if mode == 'cnn':
...
class TxEncoder(nn.Module):
    ...
    def __init__(self, num_dim=8, emb_dim=128, use_features=None,
                 data_len=48, logs_len=64, seq_mode="attn_pos", char_mode="lstm"):
        """
        seq_mode: one of ['attn', 'attn_pos', 'lstm', 'cnn', 'mean']
        """
        super().__init__()
        ...
        if "data" in use_features:
            self.data_token_encoder = CharEncoder(out_dim=emb_dim, mode=char_mode)
            self.data_seq_encoder = self._build_seq_encoder(emb_dim)
            if seq_mode == "attn_pos":
                self.data_pos = nn.Parameter(torch.randn(data_len, emb_dim))

        # logs token
        if "logs" in use_features:
            self.logs_token_encoder = CharEncoder(out_dim=emb_dim, mode=char_mode)
            self.logs_seq_encoder = self._build_seq_encoder(emb_dim)
            if seq_mode == "attn_pos":
                self.logs_pos = nn.Parameter(torch.randn(logs_len, emb_dim))

    def _build_seq_encoder(self, emb_dim):
        """根据 seq_mode 构建序列建模模块"""
        if self.seq_mode in ["attn", "attn_pos"]:
            return nn.MultiheadAttention(emb_dim, num_heads=4, batch_first=True)
        elif self.seq_mode == "lstm":
            return nn.LSTM(emb_dim, emb_dim, batch_first=True)
        elif self.seq_mode == "cnn":
            return nn.Sequential(
                nn.Conv1d(emb_dim, emb_dim, kernel_size=3, padding=1),
                nn.ReLU(),
                nn.AdaptiveMaxPool1d(1)
            )
        elif self.seq_mode == "mean":
            return None
        else:
            raise ValueError(f"Unknown seq_mode: {self.seq_mode}")

    def _encode_seq(self, token_vecs, encoder, pos_emb=None):
        """根据 seq_mode 执行对应的序列建模"""
        if self.seq_mode == "attn":
            out, _ = encoder(token_vecs, token_vecs, token_vecs)
            out = out.mean(dim=1)

        elif self.seq_mode == "attn_pos":
            if pos_emb is not None:
                token_vecs = token_vecs + pos_emb[:token_vecs.size(1), :]  # 加上可学习位置编码
            out, _ = encoder(token_vecs, token_vecs, token_vecs)
            out = out.mean(dim=1)

        elif self.seq_mode == "lstm":
            _, (h, _) = encoder(token_vecs)
            out = h.squeeze(0)

        elif self.seq_mode == "cnn":
            out = encoder(token_vecs.transpose(1, 2)).squeeze(-1)

        elif self.seq_mode == "mean":
            out = token_vecs.mean(dim=1)
    ...
    def forward(self, num_feat, data_tokens, logs_tokens):
        ...
        if "data" in self.use_features:
            ...
            data_vecs = self.data_token_encoder(data_flat).view(B, T, -1)
            pos_emb = getattr(self, "data_pos", None)
            out = self._encode_seq(data_vecs, self.data_seq_encoder, pos_emb)
            outs.append(out)

        if "logs" in self.use_features:
            B, T, L = logs_tokens.shape
            logs_flat = logs_tokens.view(B * T, L)
            logs_vecs = self.logs_token_encoder(logs_flat).view(B, T, -1)
            pos_emb = getattr(self, "logs_pos", None)
            out = self._encode_seq(logs_vecs, self.logs_seq_encoder, pos_emb)
            outs.append(out)
...
# ====== 训练函数 ======
def train_model(use_features=None, seq_mode='attn', char_mode="lstm",
    ...
    encoder = TxEncoder(num_dim=len(train_set.num_features[0]), use_features=use_features, seq_mode=seq_mode,
                        char_mode=char_mode)
    ...
    for epoch in range(epochs):
        ...
        torch.save(model, f"./models/model_epoch_{epoch}.pth")
...
def only_test(model_epoch, test_path, use_features):
    test_set = TxDataset(test_path)
    print("data loaded...", use_features)
    print("testing set: ", len(test_set))
