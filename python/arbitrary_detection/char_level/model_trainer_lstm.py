...
class TxEncoder(nn.Module):
    def __init__(self, num_dim=8, emb_dim=128, use_features=None,
        ...
        if "data" in use_features:
            ...
            self.data_pos = nn.Parameter(torch.randn(data_max, emb_dim))
        ...
        if "logs" in use_features:
            ...
            self.logs_pos = nn.Parameter(torch.randn(logs_max, emb_dim))
    ...
    def forward(self, num_feat, data_tokens, logs_tokens):
        ...
        if "data" in self.use_features:
            ...
            data_token_vecs = data_token_vecs + self.data_pos[:data_token_vecs.size(1), :]
        ...
        if "logs" in self.use_features:
            ...
            logs_token_vecs = logs_token_vecs + self.logs_pos[:logs_token_vecs.size(1), :]
...
if __name__ == "__main__":
    ...
    model = train_model(pos_csv, neg_csv, use_features=["num"], epochs=20)

    # only test 0-6
    # only_test(5, "../eth_dataset/datasets/test.csv", ["num", "data", "logs"])
