...
var (
	simulationGauge = metrics.NewRegisteredGauge("miner/simulations", nil)
)
...
func (miner *Miner) SimulateBundle(parent *types.Header, bundle *types.Bundle, rpcBribeAddress common.Address) (*big.Int, *define.SseBundleData, error) {
	...
	s, b, err := miner.worker.simulateBundles(env, []*types.Bundle{bundle}, rpcBribeAddress)
	if err != nil {
		return nil, nil, err
	}

	if len(s) == 0 {
		return nil, nil, errors.New("no valid sim result")
	}

	return s[0].RpcBundlePrice, b[0], nil
}

func (miner *Miner) SimulateBundle1(bundle *types.Bundle) (*big.Int, error) {
	...
	//_, err := miner.prepareSimulationEnv(parent, parentState)
	//if err != nil {
	//	return nil, err
	//}
...
func (miner *Miner) ExecuteBundle(parent *types.Header, bundle *types.Bundle, rpcBribeAddress common.Address) (*big.Int, *define.SseBundleData, error) {
	simulationGauge.Inc(1)
	timestamp := time.Now().Unix()
	if parent.Time >= uint64(timestamp) {
		timestamp = int64(parent.Time + 1)
	}
	...
	s, b, err := miner.worker.WorkerExecuteBundles(env, []*types.Bundle{bundle}, rpcBribeAddress)
	...
	if err != nil {
		...
		if bundle.State == types.BundleErr {
		...
		b[0].NextBlockNumber = header.Number.Uint64()
		return s[0].RpcBundlePrice, b[0], nil
	}
}
