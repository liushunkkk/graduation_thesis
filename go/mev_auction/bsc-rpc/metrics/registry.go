...
func (r *StandardRegistry) GetAll() map[string]map[string]interface{} {
	...
	r.Each(func(name string, i interface{}) {
		...
		case Histogram:
			h := metric.Snapshot()
			ps := h.Percentiles([]float64{0.5, 0.75, 0.9, 0.95, 0.99, 0.999})
		...
		case Timer:
			t := metric.Snapshot()
			ps := t.Percentiles([]float64{0.5, 0.75, 0.9, 0.95, 0.99, 0.999})
...
func (r *StandardRegistry) loadOrRegister(name string, i interface{}) (interface{}, bool, bool) {
	switch i.(type) {
	case Counter, CounterFloat64, Gauge, GaugeFloat64, GaugeInfo, Healthcheck, Histogram, Meter, Timer, ResettingTimer, Label, MyHistogram:
	default:
