...
var (
	...
	typeMyHistogramTpl            = "# TYPE %s histogram\n"
	keyValueTpl                   = "%s %v\n\n"
	appendKeyValueTpl             = "%s %v\n"
	keyQuantileTagValueTpl        = "%s {quantile=\"%s\"} %v\n"
	keyLessThenOrEqualTagValueTpl = "%s {le=\"%v\"} %v\n"
...
func (c *collector) Add(name string, i any) error {
	...
	case metrics.Label:
		c.addLabel(name, m)
	case metrics.MyHistogram:
		c.addMyHistogram(name, m.Snapshot())
	default:
...
func (c *collector) addHistogram(name string, m metrics.HistogramSnapshot) {
	pv := []float64{0.5, 0.75, 0.9, 0.95, 0.99, 0.999, 0.9999}
	ps := m.Percentiles(pv)
...
func (c *collector) addMyHistogram(name string, m metrics.MyHistogramSnapshot) {
	buckets, ps := m.LessThenOrEquals(nil)
	c.buff.WriteString(fmt.Sprintf(typeMyHistogramTpl, mutateKey(name)))
	for i := range buckets {
		c.writeMyHistogramLessThenOrEqual(name, strconv.FormatFloat(buckets[i], 'f', -1, 64), ps[i])
	}
	c.writeMyHistogramLessThenOrEqual(name, "+Inf", ps[len(ps)-1])
	c.writeMyHistogramCounter(name, m.Count())
	c.writeMyHistogramSum(name, m.MySum())
	c.buff.WriteRune('\n')
}

func (c *collector) writeMyHistogramCounter(name string, value interface{}) {
	name = mutateKey(name + "_count")
	c.buff.WriteString(fmt.Sprintf(appendKeyValueTpl, name, value))
}

func (c *collector) writeMyHistogramSum(name string, value interface{}) {
	name = mutateKey(name + "_sum")
	c.buff.WriteString(fmt.Sprintf(appendKeyValueTpl, name, value))
}

func (c *collector) writeMyHistogramLessThenOrEqual(name, p string, value interface{}) {
	name = mutateKey(name)
	c.buff.WriteString(fmt.Sprintf(keyLessThenOrEqualTagValueTpl, name, p, value))
}
