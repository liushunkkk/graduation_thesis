...
func (r *Receipt) GetReturnData() string {
	return longestUTF8Substring([]byte(r.returnData))
}

func longestUTF8Substring(data []byte) string {
	var longest string
	var current string

	for len(data) > 0 {
		r, size := utf8.DecodeRune(data)
		if (r == utf8.RuneError && size == 1) || r < 32 { // 处理无效字符
...
func (r *Receipt) SetReturnData(data string) {
	r.returnData = data
}
