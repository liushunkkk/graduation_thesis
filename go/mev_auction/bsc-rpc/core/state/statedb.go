...
type RpcChangedAccount struct {
	BeforeNonce   uint64
	BeforeBalance *uint256.Int
	AfterNonce    uint64
	AfterBalance  *uint256.Int
}
...
func (s *StateDB) GetRpcBalanceChange() map[string][]*RpcBalanceChange {
	var accounts = make(map[string][]*RpcBalanceChange)
	//s.journal.OutEntries()
	for addr := range s.journal.dirties {
		obj, exist := s.stateObjects[addr]
		if !exist {
			continue
		...
		if obj.CodeSize() != 0 {
			...
			if s.isERC20(obj.code) {
				outputChange := obj.OutputChange()
				for key, val := range outputChange {
					accounts[key] = append(accounts[key], &RpcBalanceChange{
						TokenAddress:  obj.address.String(),
						BeforeBalance: val[0],
						AfterBalance:  val[1],
						BalanceChange: SubtractUint256(val[1], val[0]),
					})
				}
			} else {
				continue
			...
			rca := &RpcBalanceChange{}
			if obj.origin != nil {
				rca.BeforeBalance = obj.origin.Balance
			} else {
				rca.BeforeBalance = uint256.NewInt(0)
			}

			if obj.data.Balance != nil {
				rca.AfterBalance = obj.data.Balance
			} else {
				rca.AfterBalance = uint256.NewInt(0)
			}
			rca.BalanceChange = SubtractUint256(rca.AfterBalance, rca.BeforeBalance)
			accounts[addr.Hex()] = append(accounts[addr.Hex()], rca)
		}
...
// 计算两个 uint256.Int 的差值，允许负数
func SubtractUint256(a, b *uint256.Int) *big.Int {
...
func (s *StateDB) isERC20(code []byte) bool {
	// 计算 ERC20 函数的选择器并检查字节码
	selectors := [][]byte{
		getFunctionSelector("totalSupply()"),
		getFunctionSelector("balanceOf(address)"),
		getFunctionSelector("transfer(address,uint256)"),
		getFunctionSelector("approve(address,uint256)"),
		getFunctionSelector("allowance(address,address)"),
	}

	for _, selector := range selectors {
		if !containsSelector(code, selector) {
			return false // 如果缺少任何一个函数，返回 false
		}
	}
	return true
}

func getFunctionSelector(functionSignature string) []byte {
	h := sha3.NewLegacyKeccak256()
	h.Write([]byte(functionSignature))
	return h.Sum(nil)[:4] // 前四个字节
}

// 检查字节码中是否包含特定的函数选择器
func containsSelector(code []byte, selector []byte) bool {
	// 确保 code 的长度大于等于 selector 的长度
	if len(code) < len(selector) {
		return false
	}

	// 遍历 code 切片，寻找与 selector 匹配的部分
	for i := 0; i <= len(code)-len(selector); i++ {
		match := true
...
func (s *StateDB) GetAllStates() map[string]map[string]string {
	result := make(map[string]map[string]string)

	for address, _ := range s.stateObjectsPending {
		object := s.stateObjects[address]
		if object != nil && len(object.pendingStorage) > 0 {
			m := make(map[string]string)
			for k, v := range object.pendingStorage {
				m[k.Hex()] = v.Hex()
			}
			result[address.Hex()] = m
		}
	}
	return result
}
