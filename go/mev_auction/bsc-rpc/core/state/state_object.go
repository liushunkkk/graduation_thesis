...
func (s *stateObject) OutputChange() map[string][]*uint256.Int {
	var outputChange = make(map[string][]*uint256.Int)
	//fmt.Println("-------preimages")
	//for key, value := range s.db.preimages {
	//	fmt.Println(key.Hex(), hex.EncodeToString(value))
	//}
	//fmt.Println("-------dirtyStorage")
	for key, value := range s.dirtyStorage {
		//fmt.Println(key.Hex(), hex.EncodeToString(value.Bytes()))
		originValue := s.originStorage[key]
		addressBytes, ok := s.db.preimages[key]
		if !ok {
			//fmt.Println("not found address in preimages")
		} else {
			changeAddress := common.BytesToAddress(addressBytes[:32]).Hex()
			var beforeValue, afterValue *uint256.Int
			bint := big.NewInt(0).SetBytes(originValue.Bytes())
			beforeValue = uint256.NewInt(0).SetBytes(bint.Bytes())
			aint := big.NewInt(0).SetBytes(value.Bytes())
			afterValue = uint256.NewInt(0).SetBytes(aint.Bytes())

			outputChange[changeAddress] = []*uint256.Int{beforeValue, afterValue}
			//fmt.Printf("changeAddress: %s, beforeValue: %s, afterValue: %s", changeAddress, beforeValue, afterValue)
		}
	}
	return outputChange
}
