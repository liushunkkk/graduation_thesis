...
// SendMevBundle sends a bundle
func (ec *Client) SendMevBundle(ctx context.Context, args types.SendMevBundleArgs) (common.Hash, error) {
	var hash common.Hash
	err := ec.c.CallContext(ctx, &hash, "eth_sendMevBundle", args)
	if err != nil {
		return common.Hash{}, err
	}
	return hash, nil
...
func (ec *Client) TraceTransaction(ctx context.Context, hash common.Hash, address common.Address) (interface{}, error) {
	var r interface{}
	err := ec.c.CallContext(ctx, &r, "scutum_balanceChangeByTx", hash, address)
	if err == nil {
		if r == nil {
			return nil, ethereum.NotFound
		}
...
func (ec *Client) PushRpcInfo(ctx context.Context, args *rpcpb.GetRpcInfoResponse) error {
	return ec.c.CallContext(ctx, nil, "scutum_pushRpcInfo", args)
}

func (ec *Client) GetRpcTransaction(ctx context.Context, txHash common.Hash) (*types.RpcTransactionResult, error) {
	var result types.RpcTransactionResult
	err := ec.c.CallContext(ctx, &result, "scutum_getRpcTransaction", txHash)
	if err != nil {
		return nil, err
	}
	return &result, nil
