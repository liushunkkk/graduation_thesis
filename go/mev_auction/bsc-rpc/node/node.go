...
func New(conf *Config) (*Node, error) {
	...
	node.pushServer = &push.SSEServer{IPLimitCount: conf.IPLimitCount}
	node.RegisterHandler("subscribe", "/stream", node.pushServer)
	node.RegisterHandler("healthCheck", "/healthCheck", &common.HealthChecker)

	invalid_tx.Server = invalid_tx.New()
	return node, nil
}
...
func (n *Node) Start() error {
	...
	invalid_tx.Server.Start()

	portal.NewRpcUserServer().Start()
...
func (n *Node) GetPushServer() *push.SSEServer {
	return n.pushServer
}
...
func (n *Node) Close() error {
	...
	portal.UserServer.Stop()

	invalid_tx.Server.Stop()
