...
// shut down.
// also test whether portal can get user information
func TestLifecycleStartupError(t *testing.T) {
	c := testNodeConfig()
	c.PortalAddress = "10.1.9.77:8000"
	stack, err := New(c)
	if err != nil {
		t.Fatalf("failed to create protocol stack: %v", err)
	}
	...
	r := portal.UserServer.GetRpcInfoList("1826863360814092288")
	if r == nil {
		t.Fatalf("failed to get rpc info")
	}
	fmt.Println(r)
