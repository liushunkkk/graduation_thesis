...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	ctx = context.WithValue(ctx, "URL", r.URL.Path)
	query := r.URL.Query()
	if query.Get("RefundAddress") != "" {
		ctx = context.WithValue(ctx, "RefundAddress", query.Get("RefundAddress"))
	}
	if query.Get("RefundPercent") != "" {
		ctx = context.WithValue(ctx, "RefundPercent", query.Get("RefundPercent"))
	}
