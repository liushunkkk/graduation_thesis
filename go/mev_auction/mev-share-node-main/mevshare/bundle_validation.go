...
// validateBundleInner 返回bundle的hash和交易总数
func validateBundleInner(level int, bundle *SendMevBundleArgs, currentBlock uint64, signer types.Signer) (hash common.Hash, txs int, unmatched bool, err error) { //nolint:gocognit,gocyclo
	...
	for i, el := range bundle.Body {
		// unmatched 指的是只有bundle中第一交易的类型为 单只有一个hash，也就是只有一个交易hash
		// 包含这种交易的，不支持simulation
		if el.Hash != nil {
...
// ValidateBundle 验证 bundle 比如版本信息，blockNumber是否合适，body信息是否正确等等
// 返回bundle的hash bundle hash = body 中所有交易的 hash 汇总后的 hash（如果有嵌套bundle，递归处理）
