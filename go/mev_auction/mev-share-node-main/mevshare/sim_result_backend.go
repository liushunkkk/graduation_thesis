...
func NewSimulationResultBackend(log *zap.Logger, hint HintBackend, builders BuildersBackend, store Storage, shareGasUsed, shareMevGasPrice bool) *SimulationResultBackend {
...
func (s *SimulationResultBackend) SimulatedBundle(ctx context.Context, bundle *SendMevBundleArgs, sim *SimMevBundleResponse, info simqueue.QueueItemInfo, shouldCancel, isOldBundle bool) error {
	...
	go func() {
		...
		if sim.Success {
			if !knownBundle {
				start := time.Now()
				// 模拟完了，要通知队列,这里应该是推流，别人需要订阅 redis
...
// ProcessHints 处理信息
// s.hint.NotifyHint(ctx, &extractedHints) 会通知 redis 队列，发布一个模拟成功消息，
func (s *SimulationResultBackend) ProcessHints(ctx context.Context, bundle *SendMevBundleArgs, sim *SimMevBundleResponse) error {
	...
	block := CurrentHeader.Number.Uint64()
