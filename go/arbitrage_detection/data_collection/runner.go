...
func RunComparisonDatasetCollectionV3() {
	ctx := context.Background()
	// 先查出所有的 block_number
	var blockNumberStrs []string
	tx := DB.Table(Table_EthereumTransactions).Distinct("block_number").Find(&blockNumberStrs)
	...
	collectOne := func(bn int) error {
		...
		maxIndex := 0
		for _, trans := range currTxs {
			var recei *EthereumReceipt
			_ = DB.Table(Table_EthereumReceipts).Where("tx_hash", trans.TxHash).Find(&recei)
			if int(recei.TransactionIndex) > maxIndex {
				maxIndex = int(recei.TransactionIndex)
			}
		}
		...
		for _, target := range transactions {
			...
			if int(receipt.TransactionIndex) > maxIndex {
				fmt.Println("结束，达到最大值：", maxIndex)
				break
			}
			...
			err = DB.Transaction(func(tx *gorm.DB) error {
				tx1 := tx.Table("test_comparison_transactions_2").Create(comparisonTransaction)
				if tx1.Error != nil || tx1.RowsAffected == 0 {
					return tx.Error
				}
				tx2 := tx.Table("test_comparison_receipts_2").Create(comparisonReceipt)
				if tx2.Error != nil || tx2.RowsAffected == 0 {
					return tx.Error
				}
				fmt.Printf("[%s] comparison transaction inserted!\n", target.Hash().Hex())
